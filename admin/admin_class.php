...
class Action
{
	...
	function login()
	{
		...
		// check for bid expired products 
		$products = $this->db->query("SELECT * FROM products WHERE bid_active=1 ");

		// set bid_active to false if the product bid date time has excedeed
		foreach ($products as $product) {
			$bidEndDatetime = $product['bid_end_datetime'];

			if ($currentDatetime > $bidEndDatetime) {

				// Update bid_active to false for expired products
				$productId = $product['id'];
				$this->db->query("UPDATE products SET bid_active = 0 WHERE id = $productId");

				// get bids
				$bids = $this->db->query("SELECT * FROM bids WHERE product_id = $productId AND bid_amount = ( SELECT MAX(bid_amount) FROM bids WHERE product_id = $productId)");
				$highestBid = $bids[0];

				$userId = $highestBid['user_id'];

				$message = "Congratulations! You have won the bid for {$product['name']}.";

				// Insert entry into messages table
				$this->db->query("INSERT INTO messages (user_id, message) VALUES ($userId, '$message')");
			}
		}

		extract($_POST);
		$qry = $this->db->query("SELECT * FROM admins where username = '" . $username . "' and password = '" . md5($password) . "' ");
		if ($qry->num_rows > 0) {
	...
	function login2()
	{
		...
		foreach ($products as $product) {
			...
			if ($currentDatetime > $bidEndDatetime) {
				...
				// get bids
				$bid = $this->db->query("SELECT * FROM bids WHERE product_id = $productId AND bid_amount = ( SELECT MAX(bid_amount) FROM bids WHERE product_id = $productId)");
				$bid_details = mysqli_fetch_array($bid);

				$userId = $bid_details['user_id'];
